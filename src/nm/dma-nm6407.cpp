...
struct LinkPortData{
    int *base;    
    LinkInfo *info;
    DtpAsync *current_cmd;    
};

struct LinkInfo{    
    LinkPortData receive_data[3];
    LinkPortData transfer_data[3];
};
...
static int linkImplSend(void *com_spec, DtpAsync *cmd){
    LinkPortData *info = (LinkPortData *)com_spec;
    
    if(info->current_cmd != 0) return DTP_AGAIN;        
    if((int)cmd->buf > 0x40000) return DTP_ERROR;    
    if((int)cmd->nwords & 1) return DTP_ERROR;    
...
static int linkImplGetStatus(void *com_spec, DtpAsync *cmd){    
    LinkPortData *info = (LinkPortData *)com_spec;    
    if(cmd == info->current_cmd){
        
        cmd->DTP_ASYNC_PRIVATE_FIELDS.status = dmaLinkStatus2DtpStatus(info->base);
        if(cmd->DTP_ASYNC_PRIVATE_FIELDS.status == DTP_ST_DONE){
            if(info->current_cmd->callback) info->current_cmd->callback(info->current_cmd->cb_data);
            info->current_cmd = 0;  
        }
    }    
...
int dtpNm6407SetDmaMutex(int *shared_mutex){
    if((int)shared_mutex < 0x40000) return DTP_ERROR;
...
int dtpNm6407Link(int desc, int port, int flags){
    ...
    info->transfer_data[port].base = (int*)0x40001800 + 0x400 * port;
    info->receive_data[port].base = (int*)0x40001810 + 0x400 * port;
    info->transfer_data[port].base[INTERRUPT_MASK] = 3;
    info->receive_data[port].base[INTERRUPT_MASK] = 3;
    info->transfer_data[port].current_cmd = 0;
    info->receive_data[port].current_cmd = 0;
    ...
    if(direction == DTP_WRITE_ONLY){
        return dtpBind(desc, info->transfer_data + port, &impl);    
    } else if(direction == DTP_READ_ONLY){
        return dtpBind(desc, info->receive_data + port, &impl);
    } else {
        return DTP_ERROR;
    }
...
static inline void linkHandlerNm6407Transfer0(){
    LinkInfo *info = &link_info;
    if(info->transfer_data[0].current_cmd->callback){
        info->transfer_data[0].current_cmd->callback(info->transfer_data[0].current_cmd->cb_data);
    }
    int *interrupt_mask = dma_state.base_tr + 0x0C;
    *interrupt_mask = 3;

    info->transfer_data[0].current_cmd = 0;
}

static inline void linkHandlerNm6407Receive0(){
    LinkInfo *info = &link_info;
    if(info->receive_data[0].current_cmd->callback){
        info->receive_data[0].current_cmd->callback(info->receive_data[0].current_cmd->cb_data);
    }
    int *interrupt_mask = dma_state.base_tr + 0x1C;
    *interrupt_mask = 3;

    info->receive_data[0].current_cmd = 0;
}

static inline void linkHandlerNm6407Transfer1(){
    LinkInfo *info = &link_info;
    if(info->transfer_data[1].current_cmd->callback){
        info->transfer_data[1].current_cmd->callback(info->transfer_data[1].current_cmd->cb_data);
    }

    int *interrupt_mask = dma_state.base_tr + 0x0C + 0x400;
    *interrupt_mask = 3;

    info->transfer_data[1].current_cmd = 0;
}

static inline void linkHandlerNm6407Receive1(){
    LinkInfo *info = &link_info;
    if(info->receive_data[1].current_cmd->callback){
        info->receive_data[1].current_cmd->callback(info->receive_data[1].current_cmd->cb_data);
    }
    int *interrupt_mask = dma_state.base_tr + 0x1C + 0x400;
    *interrupt_mask = 3;

    info->receive_data[1].current_cmd = 0;
}

static inline void linkHandlerNm6407Transfer2(){
    LinkInfo *info = &link_info;
    if(info->transfer_data[2].current_cmd->callback){
        info->transfer_data[2].current_cmd->callback(info->transfer_data[2].current_cmd->cb_data);
    }
    int *interrupt_mask = dma_state.base_tr + 0x0C + 2 * 0x400;
    *interrupt_mask = 3;

    info->transfer_data[2].current_cmd = 0;
}

static inline void linkHandlerNm6407Receive2(){
    LinkInfo *info = &link_info;
    if(info->receive_data[2].current_cmd->callback){
        info->receive_data[2].current_cmd->callback(info->receive_data[2].current_cmd->cb_data);
    }
    int *interrupt_mask = dma_state.base_tr + 0x1C + 2 * 0x400;
    *interrupt_mask = 3;

    info->receive_data[2].current_cmd = 0;
}
